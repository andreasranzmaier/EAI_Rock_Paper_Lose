...
// Number of CPU threads to use. The Raspberry Pi 3 has 4 Cortex-A53 cores.
...
TfliteClassifier::TfliteClassifier(const std::string& model_path) {
  ...
  num_classes_ = TensorElementCount(out);  // batch is 1
...
ClassifyResult TfliteClassifier::Classify(const std::uint8_t* rgb,
  ...
  const std::size_t expected =
      static_cast<std::size_t>(in_w_) * in_h_ * in_c_;
  if (rgb == nullptr || rgb_size < expected) {
    error_ = "Input buffer too small for model input.";
    return result;
  }
  ...
  switch (in->type) {
    case kTfLiteFloat32: {
      // Model includes its own rescaling layer: feed raw 0-255 pixels.
